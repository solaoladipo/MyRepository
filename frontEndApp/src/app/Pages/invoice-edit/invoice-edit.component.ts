...
export class InvoiceEditComponent {
  ...
  getVoucher(){
    ...
    let paymentCode:string = this.formHead.value.voucherCode;

    this.payService.GetPaymentVoucherByCode(paymentCode).subscribe({
      next: (res)=>{
        this.formHead.controls['BeneficiaryCode'].setValue(res.beneficiaryCode);
        this.formHead.controls['TransDate'].setValue(res.transDate);
        this.formHead.controls['Documentno'].setValue(res.documentno);
        this.formHead.controls['currency'].setValue(res.currency);
        this.formHead.controls['AnotherCharges'].setValue(res.anotherCharges);
        this.formHead.controls['EarlierAdditionorDeduction'].setValue(res.earlierAdditionorDeduction);
        this.formHead.controls['TotalAmount'].setValue(res.totalAmount);
        res.detail.forEach((x:any)=>{
          this.details.push({"Particulars": x.particulars, "ActualAmount":x.actualAmount, "TransactionFee": x.transactionFee, "WHT": x.wht, "VAT": x.vat,"NetAmount": x.netAmount, "Allocate": x.allocate});
        });
  ...
  saveRecordEdit(): void{
   ...
   if(this.formHead.value.AnotherCharges == "Earlier Addition"){
    ...
    newPaymentVoucher.Datecreated = this.formHead.value.TransDate;
    ...
    newPaymentVoucher.Refno = this.formHead.value.voucherCode;
    ...
    this.payService.saveVoucherEdit(newPaymentVoucher).subscribe({
      next: (res)=>{
